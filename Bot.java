...
class Order 
{
	   ...
	   public Order(String s, String d, int p, int si, long c) {
		   symbol = s; dir = d; price = p; size = si; count = c;
	   }
};
public class Bot
{
   ...
   public static void add(String symbol, String dir, int price, int size) throws Exception {
	   if(money-price<moneycap) {
		   return;
	   }
	    String command = "ADD "+id+" "+symbol+" "+dir+" "+price+" "+size;
   ...
   public static void convert(String symbol, String dir, int size) throws Exception {
	   String command = "CONVERT "+id+" "+symbol+" "+dir+" "+size;
	   to_exchange.println(command);
       System.err.printf("Told exchange to %s\n",command);
       id++;
  }
   ...
   public static void main(String[] args)
    {
        try {
          ...
          for(int i = 0; i < orders.size();i++) {
        	  if((orders.get(i).count > 500+count) && (orders.get(i).dir.equals("BUY"))) {
        		  cancel(i);
        	  }
          }
	        while((line = from_exchange.readLine()) != null) {
	        	...
	        	if(line.startsWith("FILL")) {
	                ...
	                if(!orders.containsKey(splited[1])) {System.err.printf("??????????????????");}
	                Order o = orders.get(splited[1]);
	                if(splited[3].equals("BUY")) {
	                	orders.get(splited[1]).num_filled += Integer.parseInt(splited[5]);
	                	money -= o.price;
	                	add(o.symbol,"SELL",(int)(o.price*(1+spread)),o.num_filled);
	                }
	                if(splited[3].equals("SELL")) {
	                	money += (int)(o.price*1.015);
	                }
	        	}
	        	if(line.startsWith("ACK") || line.startsWith("REJECT") || line.startsWith("ERROR")) {
	                System.err.printf("The exchange replied: %s\n", line);
	        	}
	        	if(line.startsWith("TRADE")) {
	        		String[] splitedTrade = line.split(" ");
	        		String symbol = splitedTrade[1];
	        		if(symbol.equals("BOND")) continue;
                  ...
                  if(history.containsKey(symbol)) {
                    ...
                    if(history.get(symbol).size() == history_size) {
                    	if(strictlyOrdered(history.get(symbol), "GREATER")) {
                    		...
                    		add(symbol,"BUY",(int)(price*(1-0.005)),buyamount);
                    	}
                    }
                  } else {
